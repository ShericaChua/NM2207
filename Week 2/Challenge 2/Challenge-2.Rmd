...
::: {.alert .alert-info}
<strong>Welcome!</strong> Hope you have watched the lecture videos and followed the instructions in code-along. Go through the steps described below, *carefully*. It is totally fine to get stuck - <strong>ASK FOR HELP</strong>; reach out to your friends, TAs, or the discussion forum on Canvas.
...
Using the data-set you are required to complete some tasks that are listed subsequently. All these tasks are based on the concepts taught in the video lectures. The questions may not be entirely covered in the lectures; To complete them, you are encouraged to use Google and the resources therein. <br>
...
**Solution:** Comma Separated Values
...
*From now on, you can use the name of the variable to view the contents of the data-set* <br> <br>
...
knitr::include_graphics("readhelp.jpg")
```
...
**Solution:** skip the rows with missing values <br> <br>
...
After learning to import a data-set, let us explore the contents of the data-set through the following questions <br> <br>
...
ggplot(data=playlist_data) + aes(x=Age,y=Rating) +
geom_point(shape = 3, colour = "black", fill = "white", size = 5, stroke = 5)
...
ggplot(data=playlist_data) + aes(x=Age,y=Rating) +
geom_point(shape = 3, colour = "black", fill = "white", size = 5, stroke = 5)+labs(title="Age against Rating", caption="This tells you the Age and Rating of DJs")
